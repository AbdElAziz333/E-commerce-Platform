...
public class InternalRegistrationController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<String>> createUser(@RequestBody RegistrationRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("User Registered Successfully", service.createUser(request)));
    }

    @PostMapping("/verify-otp")
    public ResponseEntity<ApiResponse<AuthUserDto>> verifyOtp(@RequestBody OtpVerificationRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Verified OTP Successfully", service.verifyOtp(request)));
    }
}